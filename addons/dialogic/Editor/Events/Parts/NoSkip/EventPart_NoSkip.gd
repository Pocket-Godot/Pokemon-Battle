...
func _ready():
	autoadvance_time.connect("value_changed", Callable(self, "_on_SecondsSelector_value_changed"))
	noskip_selector.connect("toggled", Callable(self, "_on_HideDialogBox_toggled"))
...
func load_data(data:Dictionary):
	...
	noskip_selector.button_pressed = event_data.get('block_input', true)
